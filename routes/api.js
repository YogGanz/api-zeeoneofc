...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6285162778904 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6285162778904 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------CIOPSBOT'+'CIOPSBOT';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'CIOPSBOT') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'CIOPSBOT') return res.json(loghandler.invalidKey)
...
router.get('/kuis/tebakgambar', async (req, res, next) => {
	...
	if(apikeyInput != 'CIOPSBOT') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://docs-api-CIOPSBOT.herokuapp.com/api/quote?type=tebakgambar`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
